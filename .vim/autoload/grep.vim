...
function! grep#parse_args(base, search, ...) abort
  ...
  let paths = [a:base > 1 ? @% : a:base > 0 ? expand('%:h') : getcwd()]
  let paths = a:0 ? copy(a:000) : paths
...
function! grep#call_grep(grep, base, depth) abort
  ...
  call call(func, [0, a:base, a:depth, search])
endfunction
...
function! grep#call_ag(bang, base, depth, ...) abort
  let flags = '--path-to-ignore ~/.ignore --path-to-ignore ~/.wildignore --skip-vcs-ignores --hidden'
  let [cmd, extra] = call('grep#parse_args', [a:base] + a:000)
  let extra .= a:depth ? ' --depth ' . (a:depth - 1) : ''
  " let opts = a:base > 0 ? {'dir': expand('%:h')} : {}
...
function! grep#call_rg(bang, base, depth, ...) abort
