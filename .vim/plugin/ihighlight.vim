...
if (!exists('g:highlightMarks_cterm_colors'))
   ...
   " Prefer this for default; just to quickly highlight stuff in white
   let g:highlightMarks_cterm_colors = [7]
endif
...
" endwhile
" Custom method
" Note numbered marks have special meaning so need to assign actual m to characters.
let s:ibase = char2nr('q') "10 letters from q-z
for s:i in range(10)
  if s:i==0
    let s:imap1 = '"'
    let s:imap2 = "'"
  ...
  exec 'nnoremap <silent> '.s:imap1.' :call <sid>AddMark("'.nr2char(s:ibase+s:i).'")<CR>:echo "Mark '.s:i.' was set."<CR>'
  exec 'nnoremap <silent> '.s:imap2.' `'.nr2char(s:ibase+s:i).':echo "Traveled to mark '.s:i.'."<CR>'
endfor
